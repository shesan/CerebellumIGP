...
gene_of_interest <- "TTR"
useRNAseq <- T
expression <- read_csv(here("data", "processed", paste0("BrainSpan_RNASeq_", useRNAseq,"_long_form.csv")), guess_max = 1000000)
...
ggplot(data = expression, aes(x=AgeRank, y=expression)) + geom_point() + theme_bw() +
